...
export async function GET(request: NextRequest) {
  try {
    ...
    const [payruns, latestPayrunWithSlips, users, warningUsers] = await Promise.all([
      prisma.payrun.findMany({
        include: { _count: { select: { payslips: true } } },
        orderBy: [{ year: "desc" }, { month: "desc" }, { createdAt: "desc" }],
      }),
      // Fetch latest payrun with payslips in parallel instead of re-querying
    ...
    const latestPayslips = latestPayrunWithSlips?.payslips || [];
